...
__create_uniquing_table(boolean_t lite_or_vmlite_mode)
{
	...
	uniquing_table->numPages = DEFAULT_UNIQUING_PAGE_SIZE;//256
	uniquing_table->tableSize = uniquing_table->numPages * vm_page_size;//256 * 16k ?
	uniquing_table->numNodes = ((uniquing_table->tableSize / (sizeof(mach_vm_address_t) * 2)) >> 1) << 1; // make sure it's even. 取偶
	...
	uniquing_table->max_collide = INITIAL_MAX_COLLIDE;//19
...
//	static const slot_parent slot_no_parent_refcount = 0xFFFFFFFF;				// 32 bits
	...
	/*
	 初始化的时候：__create_uniquing_table
	 uniquing_table->numPages = DEFAULT_UNIQUING_PAGE_SIZE;//256
	 uniquing_table->tableSize = uniquing_table->numPages * vm_page_size;//256 * 16k ?
	 uniquing_table->numNodes = ((uniquing_table->tableSize / (sizeof(mach_vm_address_t) * 2)) >> 1) << 1; // make sure it's even. 取偶   table_slot_t是两个指针大小
	 uniquing_table->u.table = (mach_vm_address_t *)(uintptr_t)sld_allocate_pages(uniquing_table->tableSize);
	 uniquing_table->table_address = (uintptr_t)uniquing_table->u.table;
	 uniquing_table->max_collide = INITIAL_MAX_COLLIDE;//19
	 uniquing_table->untouchableNodes = 0;
	...
	// 实际可以放的数量
	// 当对表进行扩容后，原表中的节点数量会被放在 untouchableNodes ，详见__expand_uniquing_table
	hash_index_t modulus = (uniquing_table->numNodes-uniquing_table->untouchableNodes-1);
	
	/*
		堆栈地址一般如下：  数组下标越靠前，堆栈越近
		0x1xxxxxxx
		0x1xxxxxxx
	    0x1xxxxxxx
	    .....
		0x1xxxxxxx
		所以，lcopy = count 表示从底层的调用指令开始入表
	 */
	...
	while (--lcopy >= 0) {//--是为了跳过 前面的（stack_buffer[count++] = self_thread + 1; ）吗？
		mach_vm_address_t thisPC = frames[lcopy];
		hash_index_t hash = uniquing_table->untouchableNodes + (((uParent << 4) ^ (thisPC >> 2)) % modulus);
		int32_t collisions = uniquing_table->max_collide;//每条指令地址在入表时，可以hash冲突的最大次数

		while (collisions--) {
			//table_slot_t 有2个指针大小，共16字节
			...
			if (table_slot->slots.slot0 == 0 && table_slot->slots.slot1 == 0) {
				...
				// slot之间用slot.parent保存上一个元素的hash，形成链表
				uParent = hash;
				break;
			}
			//如果不是空位
			...
			if (address == thisPC && parent == uParent) {//跟当前指令一样，就不重复入表
				uParent = hash;
...
// 堆栈日志文件是相当大的，并且在进程不存在后也是没用的，因为因为堆栈回溯的统一树只保存在内存中，而非磁盘上。
// 通常情况下，进程退出后，日志文件就应该被删掉(通过__prepare_to_log_stacks_stage2方法中的atexit(delete_log_files))
// 然而，还是有一些特殊情况atexit方法不会被调用，导致日志保留：
// 	- 如果进程崩溃了，或者被强杀了
//  - 如果APP支持突然中止，并被中止了
//  - 比如一个shell进程执行了一个二进制文件，并把进程pid传到了另一个不同的进程：新的进程会得有一个新的日志文件，但是老的日志文件也依然存在
// 所以，要收割死进程的日志，如果当前进程发现了一个不同于__stack_log_file_path__路径的日志文件，也要收割
...
flush_data(void)
{
	...
	pre_write_buffers->next_free_index_buffer_offset = 0;//缓存区存放的大小重置
}
...
__prepare_to_log_stacks(boolean_t lite_or_vmlite_mode)
{
	if (!pre_write_buffers) {
		...
		logging_use_compaction = (stack_logging_dontcompact ? 0 : logging_use_compaction);//默认压实
		...
		// 创建一块虚拟内存区域用于持有预写索引和栈缓冲区，这个虚拟内存区域的地址会被编码进日志文件名中。
		// 这样，当一个远程分析进程在跑这个堆栈日志文件读取代码时，就可以找到这块内存，并且映射到自己的进程内存空间中。
		// 这样，就算是最近分配内存的日志，也能被分析进程读取到。（我觉得：比如一些还没持久化到文件的日志）
		// 远程进程需要挂起这个被检查的进程以确保这些缓冲区在检查时不会改变。
		//
		// 我们需要使用shm_open()去创建共享内存区域，但是自从任何有沙盒限制的进程在执行这段代码时都不被允许创建共享内存区域后，
		// 我们换了一种方式，改用“创建一个区域并把它的内存地址写到堆栈日志文件名中”
		...
		// create the backtrace uniquing table 创建保存内存分配堆栈信息的哈希表
		...
		//创建堆栈缓存区
		uint64_t stack_buffer_sz = (uint64_t)round_page(sizeof(vm_address_t) * STACK_LOGGING_MAX_STACK_SIZE);//512
		...
		// lite_mode doesn't use a file 精简模式不记录日志到文件
...
__prepare_to_log_stacks_stage2(void)
{
	...
	if (!stage2done) {
		...
		// 收割一波死进程的堆栈日志是非常好的方法，可以帮助我们减少磁盘空间的浪费。
		// 但是，有的情况下我们不能这么做
		// 比如，进程名字是 sandboxd 或者 taskgated
		// 又或者 MallocStackLoggingNoReaping 环境变量被设为非0或非no
		// ”如果sandboxd运行在MallocStackLogging功能开启环境下，进程会挂起"
		// 这是因为reap_orphaned_log_files()方法里面，有两处地方调用了sysctl()，引起进程列表的遍历，遍历又会加锁。
		// 当进程处于过渡状态时，这种情况是不能发生的
...
#if __LP64__
	mach_vm_address_t *frames = (mach_vm_address_t*)stack_buffer + num_hot_to_skip; //跳过几条指令地址
#else
		...
		uint32_t num_hot_to_skip)
{
	...
	if (type_flags & stack_logging_type_alloc) {
		...
		__prepare_to_log_stacks_stage2();//主要在做删除一些不需要的日志文件
	}

	// compaction  如果上一次记录的alloc分配地址，刚好是这一次的dealloc的地址，则这两次都不记录了
	...
	// 把当前线程分配内存的堆栈指令数据 添加到哈希表中，添加成功则会返回数据(堆栈最近那条指令)在哈希表中的下标
	if (stack_logging_mode_lite_or_vmlite) {
...
//	} stack_logging_index_event;
	...
	if (type_flags & stack_logging_type_alloc || type_flags & stack_logging_type_vm_allocate) {
		...
		if (logging_use_compaction) {//在__prepare_to_log_stacks中初始化默认压实
