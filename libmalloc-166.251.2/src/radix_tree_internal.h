...
struct radix_edge {
    unsigned label : RADIX_LABEL_BITS;//11
    unsigned index : 16;
    unsigned labelBits : 4;
    unsigned isLeaf : 1;
};// 32位  4个字节

_Static_assert(sizeof(struct radix_edge) == 4, "size of radix_edge must be 4");//边必须是4个字节
...
_Static_assert(sizeof(struct radix_node) == 8, "size of radix_node must be 8");//node必须是8个字节
...
struct radix_tree {
    ...
    uint32_t num_nodes;
    uint32_t next_free;
    struct radix_node nodes[];//一个节点8个字节
};
